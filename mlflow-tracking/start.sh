...
# conda環境をアクティベート
source activate mlflow_jupyter_env
...
# データベースのマイグレーションを実行
mlflow db upgrade postgresql://mlflow:${POSTGRES_PASSWORD}@postgres/mlflow_db

echo "Starting MLFlow server..."
# MLflow Tracking Server を起動（ポートを 5003 に変更）
mlflow server \
    --backend-store-uri postgresql://mlflow:${POSTGRES_PASSWORD}@postgres/mlflow_db \
